...
\begin{document}
\title{Quantum nondemolition measurement and spin squeezing beyond Gaussian limit using nanophotonic waveguides and atomic ensembles}
\author{Xiaodong Qi}
...
In the Faraday interaction spin squeezing protocol, we define the fiducial, coupled and transfer states by $ \ket{\uparrow}=\ket{f=4,m=4} $, $ \ket{\uparrow}=\ket{f=4,m=3} $ and $ \ket{\uparrow}=\ket{f=4,m=2} $ respectively. 
In the $ x $-basis, a set of spin operators projected onto the qutrit subspace spanned by these three states can be defined by
\begin{align}
\hat{f_x} &= \\
\hat{f_y} &= \\
\hat{f_z} &= .
