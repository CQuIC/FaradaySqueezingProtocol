...
and 
\begin{align}
\frac{\Gamma_{f'\!,rad}}{\Gamma_0} &= 1+ \sum_f |o_{jf}^{j'f'}|^2\sum_{m',m,q=m'-m} \left|C_{f'm'}^{f,m;1,q}\right|^2\frac{\mathbf{e}_q\cdot \mathrm{Im}\left[\tensor{\mathbf{G}}_{ind,rad}(\mathbf{r}',\mathbf{r}')\right]\cdot \mathbf{e}_q^*}{G_0(\mathbf{r}',\mathbf{r}')}\\
\frac{\Gamma_{f'\!,gyd}}{\Gamma_0} &= \sum_f |o_{jf}^{j'f'}|^2\sum_{m',m,q=m'-m} \left|C_{f'm'}^{f,m;1,q}\right|^2 \frac{\mathbf{e}_q\cdot \mathrm{Im}\left[ \tensor{\mathbf{G}}_{gyd}(\mathbf{r}',\mathbf{r}')\right]\cdot \mathbf{e}_q^*}{ G_0(\mathbf{r}',\mathbf{r}')}.
\end{align}
...
\begin{align}
\Delta_0=\omega-\omega_{eg}= \frac{1}{\hbar} \bra{g}\hat{\mathbf{d}}\ket{e}\cdot \mathrm{Re}\left[ \tensor{\mathbf{G}}_{ind,rad}(\br',\br';\omega_{eg})+\tensor{\mathbf{G}}_{gyd}(\br',\br';\omega_{eg})\right]\cdot \bra{e}\hat{\mathbf{d}}\ket{g}.
\end{align}
By noting the scattered electrical field $ \mathbf{E}_{scatt}(\br)=\tensor{\mathbf{G}}(\br,\br')\cdot \bra{e}\hat{\mathbf{d}}\ket{g} $, one can rewrite the normalized frequency shift to be
\begin{align}
\frac{\Delta_0}{\Gamma_0} &= \frac{1}{2} \frac{\mathrm{Re}\left[ \bra{g}\hat{\mathbf{d}}\ket{e}\cdot\left(\mathbf{E}_{ind,rad}(\br')+\mathbf{E}_{scatt,gyd}(\br')\right)\right]}{|d|^2G_0(\br',\br')}.
\end{align}
Since the scattered field is proportional to the magnitude of the dipole moment, the normalized frequency shift is independent of the dipole moment.
