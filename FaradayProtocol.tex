...
In this Appendix we give the explicit expressions for the photon scattering rates following the formalism given in~\cite{Deutsch2010a} and ~\cite{Qi2016} while also considering the modification of decay rates of atoms due to the presence of the nanophotonic structures. 
...
$\tensor{\mathbf{G}}_0=G_0\tensor{\mathbbm{1}}$ with $G_0(\mathbf{r}',\mathbf{r}';\omega_{eg})=\frac{2}{3}k_0^3$.
In presence of a waveguide, the Green's function tensor due to the radiative modes can be decomposed into the free-space or homogeneous radiation contribution and the waveguide-induced radiation contribution, that is $ \tensor{\mathbf{G}}_{rad}(\br',\br')=\tensor{\mathbf{G}}_0(\br',\br')+\tensor{\mathbf{G}}_{ind,rad}(\br',\br';\omega_{eg}) $.
...
=1+ \frac{\mathrm{Im}\left[\mathbf{e}_q^*\cdot \tensor{\mathbf{G}}_{ind,rad}(\mathbf{r}',\mathbf{r}')\cdot \mathbf{e}_q\right]}{\mathrm{Im}\left[G_0(\mathbf{r}',\mathbf{r}')\right]}.
\end{align}
...
The total rate of photon scattering by an atom in the $(f,f_x=m)$ hyperfine structure manifold states in the $x$-basis is
	\begin{equation}\label{Eq::gammaf}
		\gamma_{m}=- \frac{2}{\hbar} {\rm Im} \big[ \bra{f,m} \hat{h}_{\rm eff}\ket{f,m} \big] ,
	\end{equation}
...
\end{align}
where $\charpol = -\frac{\sigma_0}{8\pi k_0}\frac{\Gamma}{\Delta_{ff'}+i\Gamma_{f'}/2}$ is the complex polarizability.


The rate of optical pumping between clock states $\ket{f,m} \rightarrow \ket{f,\tilde{m}}$ is
	\begin{equation}\label{Eq::gammaff_Faraday}
		\gamma_{\tilde{m}m }
		=\sum_{q}\big| \bra{f,\tilde{m}} \hat{W}_q^{\tilde{m}m} \ket{f,m} \big|^2,
	\end{equation}
where $ \hat{W}_q^{\tilde{m}m} = \frac{1}{\gamma_s}\sum_{f'}\frac{\Omega/2}{\Delta_{f'f}+i\Gamma_{f'}/2}(\mathbf{e}_q^*\cdot\hat{\mathbf{D}}_{f\tilde{m}, f'm'} )(\mathbf{e}_{\rm in}\cdot \hat{\mathbf{D}}^\dagger_{f'm',fm} ) $ are the Lindblad jump operators for optical pumping between ground levels $ (f,m)\rightarrow (f,\tilde{m}) $~\cite{Deutsch2010a}.
Each jump operator $\hat{W}_q^{\tilde{m}m}$ is associated with absorption of the probe photon polarized along $ \mathbf{e}_{\rm in} $ followed by spontaneous emission of a photon with polarization $ \mathbf{e}_q $, where $q= \{0,\pm 1\}$ labels spherical basis elements for $\pi$ and $ \sigma_\pm$ transitions.

To find the dependence on the input field intensity, we define a characteristic photon scattering rate, $\gamma_s \equiv \frac{\Gamma_{avg}\Omega^2}{4\Delta_{F}^2}= \frac{\sigma_0}{A_{\rm in}}\frac{\Gamma_{avg}^2}{4 \Delta_{F}^2} \dot{N}_L $, with Rabi frequency $ \Omega=2\bra{j}|d|\ket{j'}\mathcal{E}^{(+)}_{\rm in}/\hbar $, reduced optical dipole matrix element $\bra{j}|d|\ket{j'}$, and field amplitude $ \mathcal{E}^{(+)}_{\rm in}=|\mathbf{E}_{\rm in}^{(+)}(\br')| $.
Eqs.~\eqref{Eq::gammaf} and~\eqref{Eq::gammaff_Faraday} yield,
\begin{subequations}
	\begin{align}
		\gamma_m 
		&=n_g\dot{N}_L  \sum_{f',m'} \sigma (\Delta_{ff'} ) \mathbf{u}^*_\inp(\br'_\perp)\cdot \bra{f,m} \hat{\tensor{\mbf{A}}}(fm,f'm') \ket{f,0}  \cdot \mathbf{u}_\inp(\br'_\perp)\\
		&\approx  \gamma_s \sum_{f',m'} \frac{\Delta_{F}^2}{\Delta_{ff'}^2}\sum_q \big| o_{jf}^{j'f'}C_{f',m+q}^{f,m;1, q} \big|^2 \mathbf{e}_q^* \cdot (\mathbf{e}_{\rm in}\mathbf{e}_{\rm in}^* )\cdot \mathbf{e}_q,
	\end{align}
\end{subequations}
	\begin{align}
		\gamma_{\tilde{m}m}
		&\approx \gamma_s \sum_{f',m'} \frac{\Delta_{F}^2}{\Delta_{ff'}^2}\sum_q \big| o_{j\tilde{f}}^{j'f'} o_{jf}^{j'f'}C_{f'q}^{f,\tilde{m};1 q}C_{f',m+q}^{f,m;1,q} \big|^2 \mathbf{e}_q^* \cdot (\mathbf{e}_{\rm in}\mathbf{e}_{\rm in}^* )\cdot \mathbf{e}_q,
	\end{align}
where $ \sigma (\Delta_{ff'} )  = \sigma_0 \Gamma_{avg}^2/4\Delta^2_{f' f}$ is the the scattering cross section at the probe detuning, $ C_{f',m+q}^{f,m;1, q}=\Braket{f',m+q}{f,m;1,q}$ are the Clebsch-Gordan coefficients, and
\begin{equation}
...
The polarizability related term $\hat{\tensor{\mbf{A}}}(fm,f'm')$ is mainly determined by the vector interaction between the atoms and light due to the atomic polarizability...
