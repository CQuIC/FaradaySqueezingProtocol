...
In the rotating frame, a quantum operator $ \hat{A} $ is transfered into $ \hat{A}' $ through $ \hat{A}'\rightarrow \expect{\hat{U}_B^\dagger\hat{A}\hat{U}_B }_T $, where the notation $ \expect{\cdot}_T=\frac{1}{T}\int\cdot dt $ is the time average of observables in a period $ T $.
Similarly, the density operator is transfered to the rotating frame through $ \hat{\rho}'\rightarrow \expect{\hat{U}_B^\dagger\hat{\rho}\hat{U}_B }_T $.
...
\begin{align}
\hat{h}_{\rm loss} &= -\frac{i\hbar}{2}\sum_{f'} \gamma_s \left[C_{j'ff'}^{(0)}\hat{\mathbbm{1}}+C_{j'ff'}^{(2)}(\hat{f}_{x'}^2-\frac{\hat{f}^2}{3} ) \right],
\end{align}
