...
\end{align}
where $\charpolq = -\frac{\sigma_0}{8\pi k_0}\frac{\Gamma_{f'}^q}{\Delta_{ff'}^q+i\Gamma_{f'}^q/2}$ is the complex polarizability and the polarizability operator $  \poltens=\sum_{f',q}\charpolq\hat{\tensor{\mathbf{A}}}(f,f')$ with elements of $ \hat{\tensor{\mathbf{A}}} $ given by
...
The rate of optical pumping between clock states $\ket{f,m} \rightarrow \ket{f,\tilde{m}}$ is
	...
	\end{equation}
where $ \hat{W}_q^{\tilde{m}m} = \frac{1}{\gamma_s}\sum_{f'}\frac{\Omega/2}{\Delta_{f'f}^q+i\Gamma_{f'}^q/2}(\mathbf{e}_q^*\cdot\hat{\mathbf{D}}_{ff'} )(\mathbf{e}_{\rm in}\cdot \hat{\mathbf{D}}^\dagger_{f'f} ) $ are the Lindblad jump operators for optical pumping between ground levels $ (f,m)\rightarrow (f,\tilde{m}) $~\cite{Deutsch2010a}.
...
To find the dependence on the input field intensity, we define a characteristic photon scattering rate, $\gamma_s \equiv \frac{\Gamma_0\Omega^2}{4\Delta_{F}^2}= \frac{\sigma_0}{A_{\rm in}}\frac{\Gamma_0^2}{4 \Delta_{F}^2} \dot{N}_L $.
The optical pumping dynamics of the $ j $-th atom are governed by 
\begin{align}
\left.\dt{\hat{\rho}^{(j)}}\right|_{op} &= \mathcal{D}[\hat{\rho}^{(j)}]=-\frac{i}{\hbar}\left\{\hat{h}^{\rm loss}_{\rm eff},\hat{\rho}^{(j)} \right\}_+ + \sum_{q,q',a,b,c,d}\hat{W}_q^{dc}(\br'_j)\hat{\rho}^{(j)}\hat{W}_q^{ba}(\br'_j)\\
\\
&=-\gamma_s\frac{1}{2}\sum_{a,b}\frac{\gamma_{ba}}{\gamma_s}\ket{b}\bra{a}+
\!\!\!\!\!\!\sum_{q,q',q'',a,b,c,d,f',f'',m',m''}\!\!\!\!\!\! W_{dcq}^{f''m''q''}\left(W_{abq}^{f'm'q'}\right)^*\ket{d}\bra{c}\hat{\rho}^{(i)}\ket{b}\bra{a}
\end{align}
Eqs.~\eqref{Eq::gammaf} and~\eqref{Eq::gammaff_Faraday} yield,
\begin{subequations}
	\begin{align}
		\gamma_m 
		&=n_g\dot{N}_L  \sum_{f',q,m'=m+q} \sigma (\Delta_{ff'} ) \mathbf{u}^*_\inp(\br'_\perp)\cdot \bra{f,m} \hat{\tensor{\mbf{A}}}(f,f') \ket{f,m}  \cdot \mathbf{u}_\inp(\br'_\perp)\\
