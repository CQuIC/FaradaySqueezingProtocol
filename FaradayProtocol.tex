...
\begin{document}
\title{The nanofiber and waveguide platforms are promising towards non-Gaussian spin states}
\author{Xiaodong Qi}
...
%================================================================%
\begin{abstract}
We study the strong coupling between photons and atoms that can be achieved in an optical nanofiber geometry in the dispersive regime.
We propose a quantum nondemolition measurement and spin squeezing protocol using a nanofiber with an atom ensemble trapped nearby.
We established a general theory to calculate the spin squeezing dynamics and show that $7dB$ of spin squeezing may be achievable with $2500$ atoms trapped $1.8$ times of the fiber radius from the fiber axis.
Compared to the spin squeezing protocol in free space, the nanofiber platform enables us to increase
the photon-atom coupling strength dramatically while to reduce the decoherence due to photon scattering process at the same time,
and hence can achieve a high peak spin squeezing effect.
Compared to the Birefringence protocol using the nanofiber we have studied earlier,
the Faraday interaction protocol does not require a sophisticated search for the magic frequency and can avoid the difficulty of preparing the atoms on the less desired clock state,
and hence is more robust and easy to implement.
To achieve an even stronger spin squeezing effect towards a non-Gaussian collective spin state, we also studied the possibility of using other waveguide geometries.
\end{abstract}
