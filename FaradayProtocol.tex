...
\centering
  ...
  \begin{minipage}[bt]{.45\textwidth}
    ...
    \end{minipage}}
    %\end{subfloat}
  \caption{The polarization spectroscopy setup of the QND measurement and spin squeezing protocol based on the Faraday effect (a), evolution of the light and collective spin states (b) and internal atomic state transitions in the optical pumping process (c) are illustrated in these figures. We propose an experiment with a probe initially polarized along the $ x $-axis (or $ S_1 $ state on the \Poincare sphere shown on top-left of (b)) before guided into the nanofiber or SWG while the atoms are trapped along the $ y $-axis with standard two-color optical traps. The photon number difference between the $ D $- and $ \bar{D} $-modes are measured as a Faraday rotation on the \Poincare sphere (top-right of (b)) to indicate the collective spin state and to generate the spin squeezed state (illustrated on the bottom-right diagram of (b) on the Bloch sphere). All atoms are initialized as the fiducial $ \ket{\uparrow}_x $ state (see (c)) or as the SCS state illustrated on the Bloch sphere (bottom-left of (b)) in the $ x $-basis which is naturally chosen with the local electrical field pointing on the $ x $-direction. The probe is far detuned so that we can ignore the bias magnetic field which is used to fix the quantization axis to the $ z $-direction. With $ \pi $-polarized photons observed by the atoms, the relative flux between the $ \sigma_+ $ and $ \sigma_- $ photons indicates the state of the ensemble of atoms, since the fiducial state transition does not allow a $ \sigma_- $ photon emission while other sublevels do. Spin squeezing is also realized in the process of continuous measurement as shown on the bottom-right figure of part (b). The diagram (a) is adapted from Ref.~\cite{Qi2016} for this protocol.}\label{fig:spinsqueezingschematic}
\end{figure}
