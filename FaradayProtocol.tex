...
To fully describe the spin squeezing dynamics in the process of QND measurement, we will need a set of quantities that characterize the ensemble state of the atoms. 
\begin{align}
\expect{\Delta F_\parallel^2} &= N_A \expect{\Delta f_\parallel^2}+\frac{N_A(N_A-1)}{2}\left. \expect{\Delta f_\parallel^{(i)}\Delta f_\parallel^{(j)}}_s\right|_{i\neq j}\\
\expect{\hat{F}_\perp } &= \sum_i^{N_A} \hat{f}_\perp ^{(i)}=N_A \hat{f}_\perp.
\end{align}
As have been shown earlier, the QND measurement and spin squeezing process generates pairwise entanglement among atoms~\cite{Wang2003Spin}.
We assume there is an exchange symmetry in the ensemble and bring in the spin coherence operator $\hat{\sigma}_{ba}=\ket{b}\bra{a}$, and hence the ensemble state can be characterized by the expectation value of single-body spin operators $\expect{\hat{\sigma}_{ba}}$, the symmetric two-body correlations $\expect{\Delta \sigma_{ba}^{(1)}\Delta\sigma_{dc}^{(2)} }_s$, the symmetric three-body correlations $\expect{\Delta \sigma^{(1)}_{b_1a_1}\Delta \sigma^{(2)}_{b_2a_2}\Delta \sigma^{(3)}_{b_3a_3} }_s$ and so on. Here, the superscripts indicate the ... the subscript $s$ indicates symmetric permitations... As higher-order correlations becomes negligible, one can cut off the correlation terms at a certain order.
...
\begin{align}
\mathcal{H}\left[ \hat{\rho}\right] &= 
\end{align}
and the collective Lindblad map due to photon scattering is
\begin{align}
\mathcal{L}\left[ \hat{\rho} \right] &= .
\end{align}

We choose to use the one-body and many-body correlations to characterize the spin state of the ensemble atoms is essentially because of the following reasons...

If one truncates the many-body correlations to the order $n$, the optical dynamics part of equations can be given by
\begin{align}
...
\left. \dt{\expect{\Delta \sigma_{ba}\Delta \sigma_{dc}}}\right|_{op} &=
\end{align} 
