...
\end{align}
where $ b $ is the propagation direction and $ p $ is the polarization label of the orthonormal modes.
...
\begin{align}
\hat{\mathbf{E}}^{(+)}(\br;t) &= \sqrt{ \frac{2 \pi \hbar \omega_0}{ v_g} } \left[\mathbf{u}_H(r\!_\perp,\phi) \hat{a}_H(z,t) + \mathbf{u}_V(r\!_\perp,\phi) \hat{a}_V(z,t)\right]  e^{i \beta_0 z}\\
...
&\quad\quad\quad\quad\quad\left. + \sin\theta\mathbf{u}_n(r\!_\perp,\phi) \hat{a}_n(z,t) + \cos\theta\mathbf{u}_{\bar{n}}(r\!_\perp,\phi) \hat{a}_{\bar{n}}(z,t)\right]  e^{i \beta_0 z}\\
&= \sqrt{ \frac{2 \pi \hbar \omega_0}{ v_g} } \sqrt{2}\left[\cos(\theta\!-\!\frac{\pi}{4})\mathbf{u}_n(r\!_\perp,\phi) \hat{a}_n(z,t) + \cos(\theta\!+\!\frac{\pi}{4})\mathbf{u}_{\bar{n}}(r\!_\perp,\phi) \hat{a}_{\bar{n}}(z,t)\right]  e^{i \beta_0 z}.\label{eq:Efieldop_nnbar}
\end{align}
...
\end{align}
where $ \mathbf{g}_{H,V}=[1;1] $ is the weighting coefficient of $ H $- and $ V $-modes in the field components, $ [\hat{\mathbf{E}}^{(+)}_{n,\bar{n}}(\br;t)]=\sqrt{ \frac{2 \pi \hbar \omega_0}{ v_g} }[\mathbf{u}_n(r\!_\perp,\phi) \hat{a}_n(z,t); \mathbf{u}_{\bar{n}}(r\!_\perp,\phi) \hat{a}_{\bar{n}}(z,t)]e^{i \beta_0 z} $ is the field operator vector in the $ \{n,\bar{n} \} $ basis, and hence the $ 2\times 1 $ vector of $ \mathbf{g}_{H,V}^T\cdot\mathbf{R}^{-1} $ indicates the coefficients of the two field components in the new basis calculated in Eq.\eqref{eq:Efieldop_nnbar}.
...
&\quad+ [(\frac{\sin 4\theta}{2}+\cos 2\theta) \hat{\chi}_{nn}- \frac{\sin 4\theta}{2}(\hat{\chi}_{n\bar{n}}+\hat{\chi}_{\bar{n}n}) + (\frac{\sin 4\theta}{2}-\cos 2\theta)\hat{\chi}_{\bar{n}\bar{n}}]\hat{S}_1 \nn\\
...
\end{align}
where $\hat{\boldsymbol{\chi}}_{n,\bar{n}}=[\hat{\chi}_{nn};\hat{\chi}_{n\bar{n}};\hat{\chi}_{\bar{n}n};\hat{\chi}_{\bar{n}\bar{n}}]$ is the coupling operator vector, and the spin-polarization coupling coefficient matrix $ \mathbf{K}= \mathrm{vec}[\mathbf{R}^{-1}\cdot(\mathbf{g}_{H,V}^\dagger\cdot\mathbf{g}_{H,V})\cdot \mathbf{R} ]\cdot \mathbf{M}^{-1}= (\mathbf{R}^T\otimes \mathbf{R}^T)\cdot\mathrm{vec}[\mathbf{g}_{H,V}^\dagger\cdot\mathbf{g}_{H,V}]\cdot\mathbf{M}^{-1}$, where $ \otimes $ is the sign of Kronecker product between matrices.
The notation $ \mathrm{vec}[\cdot] $ means the vecterization of a matrix by stacking its columns. 
For our case, $ \mathrm{vec}[\mathbf{g}_{H,V}^\dagger\cdot\mathbf{g}_{H,V}]=[1;1;1;1] $.
The coupling coefficient elements, $ K_{ij} $, correspond to the coupling coefficients in Eq.\eqref{eq:heff_nnbarChiS} between the spin operators in $ \hat{\boldsymbol{\chi}}_{n,\bar{n}} $ and the polarization operators in $ \hat{\mathbf{S}} $.
