...
\end{align}
where $\charpolq = -\frac{\sigma_0}{8\pi k_0}\frac{\Gamma_{f'}^q}{\Delta_{ff'}^q+i\Gamma_{f'}^q/2}$ is the complex polarizability and the polarizability operator $  \poltens=\sum_{f',q}\charpolq\hat{\tensor{\mathbf{A}}}$ with elements of $ \hat{\tensor{\mathbf{A}}} $ given by
...
The rate of optical pumping between clock states $\ket{f,m} \rightarrow \ket{f,\tilde{m}}$ is
	...
	\end{equation}
where $ \hat{W}_q^{\tilde{m}m} = \frac{1}{\gamma_s}\sum_{f',q}\frac{\Omega/2}{\Delta_{f'f}^q+i\Gamma_{f'}^q/2}(\mathbf{e}_q^*\cdot\hat{\mathbf{D}}_{f\tilde{m}, f'm'} )(\mathbf{e}_{\rm in}\cdot \hat{\mathbf{D}}^\dagger_{f'm',fm} ) $ are the Lindblad jump operators for optical pumping between ground levels $ (f,m)\rightarrow (f,\tilde{m}) $~\cite{Deutsch2010a}.
