...
In this Appendix, we will generalize the stochastic equations and optical pumping rates outlined in Refs.~\cite{Deutsch2010a} and ~\cite{Qi2016} to the Faraday protocol case in the rotating frame discussed in this paper. 
...
We calculate the guided mode Green's function tensor using the local guided modes following the proceed discussed in Ref.~\cite{Qi2016}. The radiative Green's function tensor is calculated by simulating the local unguided modes emitted by an $i$-direction ($ i=x,y,z $) orientated electric dipole as the ideal atom placed at the atom position near the waveguides using a house-made program based on the \textit{boundary element method} (BEM)~\cite{Abajo2002,GarciadeAbajo1998Relativistic} in the frequency domain. We set $ \Gamma_{f'}= (\Gamma_{f'\!\!,\,x}+\Gamma_{f'\!\!,\,y}+\Gamma_{f'\!\!,\,z})/3$ as the averaged value of decay rates of the unpolarized light emission case for this work.
...
\begin{align}
\hat{W}_{x} &= \frac{1}{\sqrt{\gamma_s}}\!\sum_{f'} \frac{\sqrt{\Gamma_{f'}}\Omega/2}{\Delta_{ff'}+i\Gamma_{f'}/2} \left[C_{ff'}^{(0)}\hat{\mathbbm{1}} + \frac{C_{ff'}^{(2)}}{6}\left(\hat{\mathbf{f}}^2-3\hat{f}_{z}^2 \right) \right]\\
\hat{W}_{y} &= \frac{1}{\sqrt{\gamma_s}}\!\sum_{f'} -\frac{i\sqrt{\Gamma_{f'}}\Omega/2}{\Delta_{ff'}+i\Gamma_{f'}/2} C_{ff'}^{(1)}\hat{f}_z  \\
\hat{W}_{z} &= 0.
\end{align}
\end{subequations}
...
\end{align}
In our simulations, we use Eq.~\eqref{eq:drhodtop_rotating} as the master equation of the optical pumping process and define the superoperator $ \mathcal{D} $ in Eq.~\eqref{eq:op_master} accordingly. 
...
Since the collective measurement operation is independent of the atomic state, the form of the QND measurement superoperators $ \mathcal{H} $ and $ \mathcal{L} $ preserve their form in the rotating frame. With all of the components of the stochastic master equation defined in Eq.~\eqref{eq:totaldrhodt} through Eq.~\eqref{op_superator}, one can derive the motion of equations of one- and two-body moments straightforwardly using the symmetric Gaussian state approximation. Some results have been given in Eq.~\eqref{eq:dsigmabadc_op} through Eq.~\eqref{eq:dsigmabadc_QND}. The equation of motion for the optical pumping dynamics of the one-body operator $ \hat{\sigma}_{ba} $, besides the other equations of motion of moments, can be trivially given by
\begin{align}
...
\end{align}
The last step is to project the expression onto the $ \expect{\hat{\sigma}_{dc}} $ basis ($ d,c\in \uparrow,\downarrow,T $) with coefficients defined by the trace factor in the equation. The two-body moments of the optical pumping equations can be calculated in a similar way. Continued from Eq.~\eqref{eq:dsigmabadc_op}, we have 
\begin{align}
...
\end{align} 
Similarly, Eqs.~\eqref{eq:dsigmaba_QND} and~\eqref{eq:dsigmabadc_QND} can be expanded easily by using $ \Delta F_z=\Delta \sum_i^{N_A} f_z^{(i)}=\sum_i^{N_A} \sqrt{\frac{f}{2}}(\Delta\sigma_{\uparrow\downarrow}^{(i)}+\Delta\sigma_{\downarrow\uparrow}^{(i)}) + \sqrt{\frac{2f-1}{2}}(\Delta \sigma_{\downarrow T}^{(i)}+\Delta\sigma_{T\downarrow}^{(i)}) $ where the subscription $ (i) $ can only be either $ (1) $ or $ (2) $ with $ 1 $ or $ N_A-1 $ copies depending on if it is about the same atom as the other joint cumulant in the two-body moment variables. In deriving Eqs.~\eqref{eq:dsigmaba_QND} and~\eqref{eq:dsigmabadc_QND}, we have used the Gaussian state assumption to write three-body moments with one- and two-body moments by vanishing the corresponding third-order cumulants of operators, for example, $ \expect{\Delta A\Delta B\Delta C }=\expect{(\hat{A}-\expect{\hat{A}} )(\hat{B}-\expect{\hat{B}})(\hat{C}-\expect{\hat{C}}) } =0$ or $ \expect{\hat{A}\hat{B}\hat{C}}=\expect{\hat{A}\hat{B}}\expect{\hat{C}}+ \expect{\hat{A}\hat{C} }\expect{\hat{B}}+\expect{\hat{B}\hat{C} }\expect{\hat{A}}-2\expect{\hat{A}}\expect{\hat{B}}\expect{\hat{C}} $. With all these tricks applied, Eqs.~\eqref{eq:dsigmaba_QND} and~\eqref{eq:dsigmabadc_QND} yield
...
\end{align}
Eventually, by combining the optical pumping and QND measurement parts of the Eqs.~\eqref{eq:dsigmaba_op_expand} through~\eqref{eq:dsigmabadc_QND_expand}, one can find a set of stochastic equations of the one- and two-body moments about a closed set of variables of $ \left\{a,b,c,d\in \{\uparrow,\downarrow,T \}\left|\expect{\hat{\sigma}_{ba}},\expect{\Delta\sigma_{ba}\Delta\sigma_{dc} }_s\right. \right\} $ in the symmetric qutrit subspace. All the coefficients in these equations can be numerically calculated on a computer. To respect the nature of the exchange symmetry of two-body moment variables, we label variables $ \expect{\hat{\sigma}_{ba}} $ as $ \expect{\hat{\sigma}_i} $ with $ i=1,2,\cdots, 9 $ for $ b$ and $a  $ counting from $ (\uparrow,\downarrow,T) $ qutrit states; the two-body moment variables can then be ordered in the symmetric subspace with $ \expect{\Delta\sigma_i\Delta\sigma_j} $ given $ j\ge i $ for the $ \expect{\!\Delta \sigma_{ba}^{(\!1\!)}\! \Delta \sigma_{dc}^{(\!2\!)}}_s $ terms satisfying $ \expect{\!\Delta \sigma_{ba}^{(\!1\!)}\! \Delta \sigma_{dc}^{(\!2\!)}}_s=\expect{\!\Delta \sigma_{dc}^{(\!1\!)}\! \Delta \sigma_{ba}^{(\!2\!)}}_s $. In the symmetric qutrit subspace, we will have $ 45 $ two-body moment equations. Also note that $ n $-body moment equations of motion can be derived in a similar manner.

With the equations of motion, the stochastic time evolution of the one- and two-body moments can be solved by evolving the equations from the initial value which can be found as follows. We treat the system to have only two atoms, and initially each atom is prepared in the $ \ket{\Psi}=\ket{\uparrow} $ state. Then the initial value of $ \expect{\hat{\sigma}_{ba} } $ and $ \expect{\Delta\sigma_i\Delta\sigma_j}=\expect{\Delta\sigma_{ba}\Delta\sigma_{dc}}_s $ can be given by
\begin{subequations}
\begin{align}
\expect{\hat{\sigma}_{ba}}&=\bra{\Psi}\hat{\sigma}_{ba}\ket{\Psi}\\
\expect{\Delta\sigma_i\Delta\sigma_j} &= (\bra{\Psi}\otimes\bra{\Psi})\frac{\hat{\sigma}_{ba}^{(1)} \otimes\hat{\sigma}_{dc}^{(2)}+\hat{\sigma}_{dc}^{(1)} \otimes\hat{\sigma}_{ba}^{(2)}}{2}(\ket{\Psi}\otimes\ket{\Psi})\nn\\
