...
\caption{ The reciprocal spin-squeezing parameter, Eq.~\eqref{eq:xi2Faraday}. (a) and~(c) are plots of $ \xi^{-2} $  as a function of time in units of the optical pumping rate $\gamma_{op}$, for the cylindrical nanofiber and square waveguide, respectively. These curves peak at time determined by the detailed balance of reduced uncertainty due to QND measurement and decoherence due to optical pumping. (b) and~(d) show the plots of the peak value $ \xi^{-2} $ as a function of $ N_A $ for the nanofiber and square waveguide correspondingly. }\label{fig:xi_rpfix_NA_t}
\end{figure}
