...
\begin{subequations}\label{Eq::StokesComponents}
	...
	\end{align}
\end{subequations}
By measuring the output Stokes vector in a polarimeter, we preform a QND measurement of a collective atomic operator to which it was entangled.  In a proper configuration, this leads to squeezing of a collectives spin.  Launching $H$-polarized light corresponds to the initial Stokes vector along $S_1$, and Faraday rotation leads to an $S_2$ component, which is measured in a polarimeter (Fig. 2a).  Taking the $H$-mode as a coherent state with amplitude $\beta_H$, the signal the polarimeter measures $\hat{S}_2^{out} = (\beta_H \hat{a}_V^{\dag out} +\beta^*_H \hat{a}_V^{out})/2$.  Expressed in this way we see that the polarimeter acts as a homodyne detector, with the input $H$-mode acting as the local oscillator and the photons scattered into the $V$-mode as the signal.  Formally, the input-out relation follows from the scattering equations, Eq. (?), and reads
\begin{equation}
...
Figure ? shows a plot of $A_{Far}$, $A_{in}$, and $C_1$ as a function of  $\mbf{r}'_\perp$ for the two nanophotonic geometries.  We see that $A_{Far}$ is cylindrically symmetric, and thus the measurement strength is independent of the azimuthal position, depending only on $\vert \mbf{r}'_\perp\vert$.  In contrast $A_{in}$ is azimuthally anisotropic.  It is largest along the $y$-axis at a given radial distance, which corresponds to smallest intensity of the $H$-mode, and thus smallest scattering rate $\gamma_s$.  This angle corresponds to the position at which $\vert \mbf{u}_V (\mbf{r}'_\perp) \vert$ is largest and thus yields the largest enhancement of $C_1$.  Thus, counterintuitively, we enhance the cooperativity by placing the atom at the angle of minimum input intensity.   This enhancement is even greater for the square waveguide as the modes are even more anisotropic in this case.  For typical geometries we find for nanofiber with radius $a = 250$nm and atoms trapped and a radial distance $?$ from the surface, the single atom cooperativity is $C_1 =?$ at the optimal trapping angle; for the square waveguide of with $w =?$nm and atoms trapped $?$nm from the surface, $C_1 =?$ at optimum.  Thus with order 1000 trapped atoms the $N_A$-atom cooperativity is of order ?, sufficient to generate substantial spin squeezing.
...
\end{align}
where the first term of Eq.~\eqref{eq:DeltaFz2} and Eq.~\eqref{eq:expectFx} is solely determined by a symmetric sum over $N_A$ identical spin-$f$ single-body operators, and  the second term of Eq.~\eqref{eq:DeltaFz2} is determined by two-body covariances, $ \left.\expect{\Delta f_\perp^{(i)}\Delta f_\perp^{(j)}}\right|_{i\neq j}=\expect{\Delta f_z^{(1)}\Delta f_z^{(2)}} \equiv \expect{\hat{f}_z^{(1)}\hat{f}_z^{(2)}}-\expect{\hat{f}_z^{(1)}} \expect{\hat{f}_z^{(1)}} $.  Negative values in these two-body correlations correspond to the pairwise entanglement among atoms that yields spin squeezing~\cite{Wang2003Spin}.
...
To study the spin squeezing dynamics, we employ a first principles based on a stochastic master equation for the collective state of $N_A$ atoms,
...
The rate of decoherence is characterized by the free space scattering rate, Eq. (?).  Note, optical pumping superator, Eq. (?),  is not trace preserving when restricted to a given hyperfine manifold $f$.  In this case, optical pumping of atoms to the other hyperfine manifold in the ground-electronic state is treated as loss.

The solution to the master equation is made possible by two approximations. Firstly, we restrict the subspace of internal magnetic sublevels that participate in the dynamics.  The systems is initialized in a spin coherent state, with all atoms spin-polarized along an axis in the transverse plane, e.g., the $x$-axis.  We denote this as the ``fiducial state" $\ket{\uparrow} = \ket{f, m_x =f}$.   Through QND measurement, spin squeezing is induced by entanglement with the  ``coupled state"  $\ket{\downarrow} = \ket{f, m_x=f-1}$.  Optical pumping is dominated by ``spin flips" $\ket{\uparrow}\rightarrow \ket{\downarrow}$ and ``loss" due to pumping to the other hyperfine level.  We finally include a third internal magnetic sublevel $\ket{T} = \ket{f, m_x=f-2}$ to account for  ``transfer of coherences" that can occur in spontaneous emission~\cite{?}.  Restricted to this qutrit basis, the internal hyperfine spin operators are
\begin{subequations}
\begin{align}
...
Secondly, we make the Gaussian approximation, valid for large atomic ensembles, so that the many-body state is fully characterized by one and two-body correlation. Equivalently, the state is defined by the one and two-body density operators, with matrix elements $\rho^{(1)}_{a, b} =\expect{\hat{\sigma}_{ba}}$, $\rho^{(1,2)}_{ac,bd}=\expect{\Delta \sigma_{ba}^{(1)}\Delta\sigma_{dc}^{(2)} }$.   Optical pumping, acting locally, couple only $n$-body correlations to themselves, e.g.,
\begin{equation}
\expect{\Delta \sigma_{ba}^{(1)}\Delta\sigma_{dc}^{(2)} }_{op} = \expect{\mathcal{D}[\Delta \sigma_{ba}^{(1)}]\Delta\sigma_{dc}^{(2)} }_{op}dt + \expect{\Delta \sigma_{ba}^{(1)} \mathcal{D}[\Delta\sigma_{dc}^{(2)}] }_{op}dt .
\end{equation}
...
\begin{align}
\left.d\expect{\Delta f_z^{(i)} \Delta f_z^{(j)}} \right|_{QND} &= \left.d\expect{\hat{f}_z^{(i)} \hat{f}_z^{(j)}} \right|_{QND} -2 \left. \expect{\hat{f}_z} \right|_{QND} \left( \left.d\expect{\hat{f}_z} \right|_{QND}\right) - \left( \left.d\expect{f_z} \right|_{QND}\right)^2 \nonumber \\
&= -\kappa\expect{\Delta f^{(i)}_z  \Delta F_z } \expect{\Delta F_z \Delta f_z^{(j)} }dt,
\end{align}
where we have employed the Ito calculus $dW^2 = dt$.

With this formalism in hand, we can calculate the squeezing parameter, Eq. (?), as a function of time by finding equations of motion for the one-body averages $\expect{\hat{f}_x}$ and  $\expect{\Delta f_z^2}$, and the two-body correlations $\expect{\Delta f_z^{(1)} \Delta f_z^{(1)}}$.  This is done in the qutrit basis according to Eq. (?).
