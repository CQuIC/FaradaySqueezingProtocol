...
\usepackage{bbm}
\usepackage{bbold}
% The following two packages are only used for editting and proofreading, and can be removed before submission.
\usepackage{siunitx}
\usepackage{ulem}
...
The Faraday effect arises from the irreducible rank-1 (vector) component of the polarizability tensor~\cite{Deutsch2010a}.  Given an atom with hyperfine spin $f$, this contribution is $\hat{\alpha}^{vec}_{ij} = i \alpha_1 \epsilon_{ijk} \hat{f}_k$, where $\alpha_1 = C^{(1)}_{f}\frac{\sigma_0}{4\pi k_0}\frac{\Gamma_A}{2\Delta} $ is the characteristic polarizability.  For alkali atoms $C^{(1)}_f=\mp\frac{1}{3f}$ for the D1- and D2-line transitions, respectively.  We take the detuning, $ \Delta $, large compared to the excited state hyperfine splitting.  The resonant scattering cross section on a unit oscillator strength is $\sigma_0 = 6\pi/k_0^2$, where $k_0=\omega_0/c$.  The polarization transformation associated with scattering from $H$ to $V$ mode is determined by the operator
\begin{equation}
...
\centering
  ...
  \caption{(a) Schematic polarization spectroscopy geometry for the QND measurement and spin squeezing generation based on the Faraday effect.  Atoms trapped near the surface of the nanophotonic waveguide cause a Faraday rotation of the guided light, which is measured in a polarimeter that detects the $S_2$ component of the Stockes vector (intensity is diagonal $D$ minus $\bar{D}$).  (b) The evolution of the light's polarization state on the \Poincare sphere  (left to right).  The Stokes vector of the light is prepared along the $S_1$ direction and for some atom spin projection along $z$, the Faraday interaction causes a rotation around the $3$-axis.  Shot noise, shown as un certainty bubble, limits the resolution of the detection. (c) The evolution of the collective state (left to right). Before and after the measurement the spin is prepared in a coherent state.  Projection noise is shown as an uncertainty bubble.  After the measurement the uncertainty in $F_z$ is squeezed, at the direction is correlated with the outcome on the polarimeter. }\label{fig:spinsqueezingschematic}
\end{figure}
