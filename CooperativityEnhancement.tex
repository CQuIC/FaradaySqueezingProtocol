...
\begin{align}
\frac{\mathrm{OD}_{\rm eff}}{N_A} \equiv \frac{\kappa}{\gamma_s}=\frac{\sigma_0A_{in}}{A_{\inter}^2},
\end{align}
...
A QND measurement protocol based on birefringence effect will require a rotation of light polarization about an axis pointing on the equator while the initial polarization state of the probe is on the orthogonal direction on the equator which is linearly polarized. 
At the polarization spectroscopy measurement end, the helicity of the light will be changed and the rotating angle can be read out via a homodyne detection by counting the photon number difference between the left- and right-polarized light signal.

We assume the input light is linearly polarized along the diagonal direction that is a $ D $- or $ S_2 $-mode input. 
A polarization rotation around $ S_1 $ axis would be ideal to generate a maximal visible result if $ S_3 $-projection polarization measurement is performed. 
The rotation of the light polarization state on the \Poincare sphere is determined by 
\begin{align}
