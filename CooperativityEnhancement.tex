...
In the rotating frame, a quantum operator $ \hat{A} $ is transferred into $ \hat{A}' $ through $ \hat{A}\rightarrow \hat{A}'=\expect{\hat{U}_B^\dagger\hat{A}\hat{U}_B }_T $, where the notation $ \expect{\cdot}_T=\frac{1}{T}\int\cdot dt $ is the time average of observables in a period $ T $.
...
\end{align}
\end{subequations}
After some algebra, one can find the optical pumping master equation in the rotating frame as
\begin{align}
...
\end{align}
\end{subequations}
With the initial values and equations of motion given above, the stochastic time evolution of the one- and two-body moments can then be solved by evolving the equations from the initial value to arbitrary time $ t $. The collective spin squeezing dynamics can be calculated using the relations derived in Appendix~\ref{Appendix::collectivespinoperators} given the one-body and two-body moments calculated in this approach.
