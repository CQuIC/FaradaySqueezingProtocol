...
\begin{align}
\left.\dt{\hat{\rho}^{(j)}}\right|_{op} &= \gamma_s\mathcal{D}[\hat{\rho}^{(j)}]=-\frac{i\gamma_s}{\hbar}\left\{\hat{H}_{\rm eff}^{(j)},\hat{\rho}^{(j)} \right\}_+ + \gamma_s\sum_{q}\hat{W}_q(\br'_j)\hat{\rho}^{(j)}\hat{W}_q(\br'_j) %\\
...
We calculate the guided mode Green's function tensor using the local guided modes following the proceed discussed in Ref.~\cite{Qi2016}. The radiative Green's function tensor is calculated by simulating the local unguided modes emitted by a $i$-th orientated electric dipole as the ideal atom placed at the atom position near the waveguides using a house-made program based on the \textit{boundary element method} (BEM)~\cite{Abajo2002,GarciadeAbajo1998Relativistic} in the frequency domain.
...
\begin{align}
\hat{h}_{\rm loss} =-\frac{i\hbar}{2} \sum_{f'}\gamma'_s \left[C_{ff'}^{(0)}\hat{\mathbbm{1}} + \frac{C_{ff'}^{(2)}}{6}(\hat{\mathbf{f}}^2-3\hat{f}_z^2 ) \right],
\end{align}
\end{subequations}
...
\begin{subequations}
\begin{align}
\hat{h}_{\rm loss} &= -\frac{i\hbar}{2} \sum_{f'} \gamma'_s(f') \sum_{m=-f}^f\left[C_{ff'}^{(0)} + \frac{C_{ff'}^{(2) }}{6}(f(f+1)-3m^2) \right]\hat{\sigma}_{mm}\\
