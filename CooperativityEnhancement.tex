...
We define the Lindblad jump operators of optical pumping among ground states by~\cite{Deutsch2010a}
	\begin{align}\label{Eq::Wq_Faraday}
		\hat{W}_q &= \frac{1}{\sqrt{\gamma_s}}\sum_{f'}\frac{\sqrt{\Gamma_{f'}}\Omega/2}{\Delta_{f'f}+i\Gamma_{f'}/2}\mathbf{e}_q^*\cdot(\hat{\mathbf{D}}_{ff'}  \hat{\mathbf{D}}^\dagger_{f'f} )\cdot\mathbf{e}_{\rm in} \\
...
&\quad\quad\quad\quad\quad\cdot\!\sum_{m=-f}^f\!\!\left[\!C_{f\!f'}^{(0)} \!+\! \frac{C_{f\!f'}^{(2) }}{6}(f(f\!+\!1)\!-\!3m^2) \right]\!\hat{\sigma}_{mm}\nn\\
