...
Figure ? shows a plot of $A_{Far}$, $A_{in}$, and $C_1$ as a function of  $\mbf{r}'_\perp$ for the two nanophotonic geometries.  We see that $A_{Far}$ is cylindrically symmetric, and thus the measurement strength is independent of the azimuthal position, depending only on $\vert \mbf{r}'_\perp\vert$.  In contrast $A_{in}$ is azimuthally anisotropic, largest along the $V$-axis  for a given radial distance, corresponds to smallest intensity of the $H$-mode, and thus smallest scattering rate $\gamma_s$.  This corresponds to the position at which $\vert \mbf{u}_V (\mbf{r}'_\perp) \vert$ is largest and position leads to the biggest enhancement of $C_1$.   Note, the position where  $\vert \mbf{u}_V (\mbf{r}'_\perp) \vert$ is largest corresponds to the position where the input mode is smallest.  Thus, counterintuitively, we enhance the cooperativity by placing the atom at the angle of minimum input intensity.  This enhancement is even greater for the square waveguide compare to the cylindrical nanofiber, as modes are even more anisotropic for the square geometry.  For typical geometries we find for nanofiber with radius $a = 250$nm and atoms trapped and a radial distance $?$ from the surface, the single atom cooperativity is $C_1 =?$ at the optimal trapping angle; for the square waveguide of with $w =?$nm and atoms trapped $?$nm from the surface, $C_1 =?$ at optimum.  Thus with order 1000 trapped atoms the $N_A$-atom cooperativity is of order ?, sufficient to generate substantial spin squeezing.
...
\end{align}
where  $\kappa$ is the measurement strength defined in Eq. (?), and $\mathrm{d}W$ is a stochastic Weiner interval satisfying $\mathrm{d}W^2 = \mathrm{d}t$. The conditional dynamics are generated by superoperators that depend on the {\em collective} spin 
\begin{subequations}
\begin{align}
...
The solution to the master equation is made possible by two approximations. Firstly, we restrict the subspace of internal magnetic sublevels that participate in the dynamics.  The systems if initialized in a spin coherent state, with all atoms spin-polarized along an axis in the transverse plane.  We denote this a the ``fiducial state" $\ket{\uparrow} = \ket{f, m\perp =f}$.   Through QND measurement, spin squeezed is induced by entanglement with the  ``coupled state"  $\ket{\downarrow} = \ket{f, m\perp =f-1}$



Firstly, we make the Gaussian approximation, valid for large atomic ensembles, so that the many-body state is full characterized by one and two-body correlation. We define the spin coherence operator for two internal magnetic sublevels $\hat{\sigma}_{ba}=\ket{b}\bra{a}$ to represent the matrix elements of one and two-body density operators $\rho^{(1)}_{a, b} =\expect{\hat{\sigma}_{ba}}$, $\rho^{(1,2)}_{ac,bd}=\expect{\Delta \sigma_{ba}^{(1)}\Delta\sigma_{dc}^{(2)} }_s$. S
...
\end{align}
\end{subequations}
The optical dynamics part can be given by
\begin{align}
