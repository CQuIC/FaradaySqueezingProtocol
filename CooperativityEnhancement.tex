...
\centering
  ...
  \end{subfloat}%  
  \caption{$H$-mode components (a) and intensity distribution (b) in the $ xy $-plane for the optical nanofiber and SWG. In \protect\subref*{fig:nanofiberswg_Hmode_E_xy}, shown on the first row, from left to right, are $ \mathrm{Re}[u_x(\br\!_\perp)] $, $ \mathrm{Re}[u_y(\br\!_\perp)] $ and $ \mathrm{Im}[u_z(\br\!_\perp)] $ of the nanofiber's $ H $-mode. White lines outline the waveguide boundary. The scale is normalized to the maximum value of all field components shown on the subplots. Plots on the second row are the same components as the first row but for the SWG. All the other parts of the electric field components of the $ H $-mode not shown in these plots for both waveguide geometries are zero everywhere. In \protect\subref*{fig:nanofiberswg_Hmode_Ints_xy}, we plot the normalized intensity distribution on the transverse plane for both geometries. The arrows indicates the local field's direction and amplitude (relative length) at positions along the vertical waveguide axis, which only have an $ x $-component of the mode. The $ V $-mode is the $ H $-mode rotated by $ 90^\circ $ around the waveguide axis. }\label{fig:nanofiberswg_E_ints}
\end{figure}
...
\end{align}
The equal sign is typically not achieved for a nanophotonic waveguide due to the out-of-phase $z $-component of the modes at arbitrary positions, while the upper bound naturally occurs in free-space, $ C_1=\frac{\sigma_0}{\Ai}=\sigma_0 u(r'_\perp)^2$ for Gaussian modes or plane waves, where $ \vert \mbf{u}_H (\mbf{r}'_\perp) \vert ^2=\vert \mbf{u}_V (\mbf{r}'_\perp) \vert ^2 = u(r'_\perp)^2 $ independent of azimuthal angle. The upper bound may also saturated with TEM modes when the longitudinal mode component vanishes. \comment{However, as we will show next, by proper choice of the atomic position for the waveguide geometry, the cooperativity can be strongly enhanced over free space, due to the tight and anisotropic confinement of the modes the waveguide guides.}
...
Figs.~\ref{fig:nanofiber_Aeffgeometry} and~\ref{fig:swg_Aeffgeometry} show plots of $1/\AF$, $1/\Ai$, and $C_1$ as a function of  $\mbf{r}'_\perp$ for the two nanophotonic geometries.  {\color{red} We see that $\AF$ is cylindrically symmetric for both nanofiber and SWG geometries which have a $ C_4 $ rotation symmetry, and thus the measurement strength is independent of the azimuthal position of the atoms.  -- ( Ivan says "huh?")}  In contrast $\Ai$ is azimuthally anisotropic.  $1/\Ai$ is smallest along the $y$-axis at a given radial distance, which corresponds to smallest intensity of the input $H$-mode, and thus smallest scattering rate $\gamma_s$.  This angle corresponds to the position at which $\vert \mbf{u}_V (\mbf{r}'_\perp) \vert$ is largest and thus yields the largest enhancement of $C_1$.  Thus, counterintuitively, we enhance the cooperativity by placing the atom at the angle of minimum input intensity.   This enhancement is even greater for the square waveguide which has more anisotropic guided modes compared to the cylindrical nanofiber.  For typical geometries, given a nanofiber with radius $a = 225$nm and atoms trapped at a radial distance $200$nm ($0.8a$) from the surface, the single atom cooperativity is $C_1 =0.23$ at the optimal trapping angle; for the square waveguide of width $w =300$nm, and atoms trapped $150$nm from the surface, $C_1 =0.32$ at optimum.  Thus with order 1000 trapped atoms the $N_A$-atom cooperativity is of order of $ 300 $, sufficient to generate substantial spin squeezing.
...
{\color{red} In the absence of any other noise, the cooperativity of atom-light coupling increases as the atoms are placed closer to the waveguide surface. Figs.\ref{fig:nanofiber_peakxi_rp_NA2500} and~\ref{fig:swg_peakxi_rp_NA2500} show the peak squeezing  as a function of $ r\!_\perp $ for both nanofiber and SWG geometries with $2500$ atoms. We find that the peak squeezing  linearly decreases in the unit of $ dB $ or logarithmically as the atoms move away from the waveguide surface. To explain this result, we also plot out the cooperativity, $ C_1 $, in the logarithm scale in Figs.\ref{fig:nanofiber_C1_y} and~\ref{fig:swg_C1_y} as a function of atom radial distance to the center of both waveguide geometries. As can be seen, $ \log_{10}(C_1) $ is also linearly decreasing as $ r\!_\perp $ increases. This result implies that cooperativity is indeed the key parameter to predict the peak squeezing  and can be employed to optimize the QND measurement and spin squeezing geometries.  -- Ivan -- It is well known that the peak squeezing scales as $\sqrt{OD}$.  We can confirm that easily, but don't need to emphasize it}
...
In this paper, we have studied the atom-light coupling based on Faraday interaction in the dispersive regime with application to QND measurement and spin squeezing with an ensemble of atoms. A set of stochastic master equations are derived by assuming the collective spins are symmetric Gaussian states while the simulations are performed in the qutrit internal subspace on a cylindrical nanofiber and a square waveguide platform, respectively. We find the cooperativity of the per atom and light coupling is the key parameter to predict the peak spin squeezing parameter, which is a result of measurement backaction competing with decoherence. By optimizing the cooperativity, the optimal measurement and trapping geometry can be defined by trapping atoms at the minimal intensity spots while the probe beam is initially polarized along the symmetry axis of the cylindrical nanofiber and square waveguide transverse planes. With realistic parameters chosen, our simulation shows that larger than $ 7 $dB for the nanofiber or $ 10 $dB for the SWG of the inversed spin squeezing parameter may be achievable with $ 2500 $ atoms. The proposed spin squeezing protocol based on Faraday interaction has shown supremacy of achievable squeezing effect over the birefringence-based spin squeezing protocol proposed in our earlier work~\ref{Qi2016}; meanwhile, compared to the birefringence protocol and the recent experimental demonstration of QND measurement using a nanofiber based on a two-color scheme~\cite{Beguin2017Observation}, this proposal also has the advantage of minimizing the side effects due to the presence of the probe light by fully utilizing the anisotropic properties of the dielectric waveguides and preferring a minimal local field strength for a considerable squeezing effect. 

Although we have only considered a cylindrical nanofiber and a square waveguide geometries, it is immediate applicable to generalize our theory to other nanophotonic waveguides and further optimize the waveguide geometry for a better QND measurement and spin squeezing effect which is the key for atomic state readout and non-classical collective spin state preparation towards versatile nanophotonic-atom quantum interfaces~\cite{Qi2016}. 

As this work focuses on illustrating the significance of cooperativity in terms of optimizing waveguide-based QND measurement and spin squeezing geometries, we have not considered the Purcell effect and the modification of decay rates in our simulations which will further complicate this problem. We have neither considered the motion of atoms in the optical lattice for the same reason. In practice, however, these effects are important as having been observed in the nanofiber experiments~\cite{Solano2017Dynamics,Solano2017Alignment,Beguin2017Observation,Solano2017Optical}. We will include these effects in our future studies towards strategies of atom cooling and state initializations on these nanophotonic platforms. 

By the nature of the QND measurement protocols, the probe light is preferably working in the far-off-resonance regime where the photon scattering among atoms is weak and can be negligible. Sandia National Labs also plan to fabricate more sophisticated nanophotonic waveguides than the two simple geometries discussed in this paper. For the completion of the theory of atom-light interactions on nanophotonic interfaces, it is necessary to include the collective effects due to photon scattering processes on general nanophotonic platforms, which will lead to the break of the symmetric state assumption which we have been using in our work~\cite{Qi2016} and will also inspire applications including quantum memories~\cite{Asenjo-Garcia2017Exponential,Asenjo-Garcia2017Atom} and matrix product state and cluster state generations~\cite{Economou2010,Lodahl2017Chiral,Schwartz2016Deterministic,Pichler2016Photonic,Pichler2017Photonic} based on the long-range interaction generated superradiance~\cite{Solano2017Super}, photonic delay lines and spin waves. The theoretical difficulty of studying these many-body effects lays down to the exponentially expanded Hilbert quantum space which cannot be stored efficiently on a classical computer. We hope the computational framework based on the $ n $-body moments will cast a light on understanding the generation of quantum correlations and state storages with an acceptable computational load. 
...
%===============Appendix================= %
\begin{appendix}
\section{Photon scattering and optical pumping rates}\label{Sec::opticalpumpinginrotatingframe}
In this appendix, we will derive the stochastic equations from the static frame to the rotating frame and provide details on how we solve the spin dynamics using the Faraday protocol with waveguides discussed in the main text. 
...
\end{align} 
We have defined a characteristic photon scattering rate, $\gamma_s \equiv \frac{\Gamma_A\Omega^2}{4\Delta_{F}^2}= \frac{\sigma_0}{A_{\rm in}}\frac{\Gamma_A^2}{4 \Delta_{F}^2} \dot{N}_L $ with an effective detuning $ \Delta_F $ defined by $ \frac{1}{\Delta_F}=\sum_{f'}\frac{C_{f'ff'}^{(1)}}{\Delta_{ff'}} $ and $ \Delta_{ff'}=\omega-\omega_{ff'} $.
...
\end{align}
and hence $ \frac{\sqrt{\Gamma_{f'}}\Omega/2}{\Delta_{ff'}\pm i\Gamma_{f'}/2}\approx \frac{\sqrt{\Gamma_{f'}}\Omega/2}{\Delta_{ff'}}=\sqrt{\gamma_s}\frac{\Delta_F}{\Delta_{ff'}} $ in the far-detuning regime.
...
We define the Lindblad jump operators of optical pumping among ground states by~\cite{Deutsch2010a}
	\begin{align}\label{Eq::Wq_Faraday}
		\hat{W}_q &= \frac{1}{\sqrt{\gamma_s}}\sum_{f'}\frac{\sqrt{\Gamma_{f'}}\Omega/2}{\Delta_{f'f}^q+i\Gamma_{f'}^q/2}\mathbf{e}_q^*\cdot(\hat{\mathbf{D}}_{ff'}  \hat{\mathbf{D}}^\dagger_{f'f} )\cdot\mathbf{e}_{\rm in} \\
		&= \frac{1}{\sqrt{\gamma_s}}\!\sum_{f'k}\! \frac{\sqrt{\Gamma_{f'}}\Omega/2}{\Delta_{ff'}+i\Gamma_{f'}/2} \left[\delta_{qx}C_{ff'}^{(0)}\hat{\mathbbm{1}} \!+\! iC_{ff'}^{(1)}\epsilon_{qxk}\hat{f}_k \!+\! C_{ff'}^{(2)}\left(\frac{\hat{f}_q\!\hat{f}_{x}\!+\!\hat{f}_{x}\!\hat{f}_q }{2} \!-\! \frac{\delta_{qx}}{3}\hat{\mathbf{f}}^2 \right) \right].
...
We calculate the guided mode Green's function tensor using the local guided modes following the eigenmode decomposition formula discussed in Ref.~\cite{Qi2016}. Using a house-made program based on the \textit{boundary element method} (BEM) in the frequency domain~\cite{Abajo2002,GarciadeAbajo1998Relativistic}, the radiative Green's function tensor is calculated by simulating the local unguided modes emitted by an $i$-directed ($ i=x,y,z $) optical dipole as the ideal atom placed near the waveguides. We set $ \Gamma_{f'}= (\Gamma_{f'\!\!,\,x}+\Gamma_{f'\!\!,\,y}+\Gamma_{f'\!\!,\,z})/3$ as the averaged value of decay rates for this study.
...
\begin{subequations}\label{eq:rotationtransf}
	...
	\end{align}
\end{subequations}
In the rotating frame, operators with a transverse atomic angular momentum will be averaged to vanish. For example, $ \hat{f}_{x}=\hat{f}_{y} \rightarrow 0 $ and $ \hat{f}_{i=x,y}\hat{f}_{z}\rightarrow 0 $.
...
\hat{W}_{x} &= \frac{1}{\sqrt{\gamma_s}}\!\sum_{f'} \frac{\sqrt{\Gamma_{f'} }\Omega/2}{\Delta_{ff'}+i\Gamma_{f'}/2 } \sum_{m=-f}^f\left[C_{ff'}^{(0)} + \frac{C_{ff'}^{(2) }}{6}(f(f+1)-3m^2) \right]\hat{\sigma}_{mm}\\
\hat{W}_{y} &= -\frac{i}{\sqrt{\gamma_s}}\!\sum_{f'} \frac{\sqrt{\Gamma_{f'} }\Omega/2}{\Delta_{ff'}+i\Gamma_{f'}/2 } \sum_{m=-f}^f C_{f'ff'}^{(1)}m\hat{\sigma}_{mm}\\
\hat{W}_z &=0.
\end{align}
\end{subequations}
...
\end{align}
In our simulations, we use Eq.~\eqref{eq:drhodtop_rotating} as the master equation of the optical pumping process in the magnetic field and define the superoperator $ \mathcal{D} $ in terms of Eq.~\eqref{eq:op_master} accordingly. 

As a sanity check, we consider a far-detuning regime where the detuning on hyperfine sublevels is indistinguishable so that we can denote $ f''$ as $f' $ and ignore all tensor polarizability terms wherever $ C_{ff'}^{(2)} $ presents, and the optical pumping part of the master equation above becomes
...
\end{align}
The last step is to project the expression onto the $ \expect{\hat{\sigma}_{dc}} $ basis ($ d,c\in \{\uparrow,\downarrow,T\} $) with coefficients defined by the trace factor in the equation above. The equations of two-body moments of the optical pumping can be derived in a similar way. Continued from Eq.~\eqref{eq:dsigmabadc_op}, we have 
\begin{align}
...
\end{align} 
Similarly, Eqs.~\eqref{eq:dsigmaba_QND} and~\eqref{eq:dsigmabadc_QND} can be expanded easily by using $ \Delta F_z=\Delta \sum_i^{N_A} f_z^{(i)}=\sum_i^{N_A} \sqrt{\frac{f}{2}}(\Delta\sigma_{\uparrow\downarrow}^{(i)}+\Delta\sigma_{\downarrow\uparrow}^{(i)}) + \sqrt{\frac{2f-1}{2}}(\Delta \sigma_{\downarrow T}^{(i)}+\Delta\sigma_{T\downarrow}^{(i)}) $ where the subscription $ (i) $ can only be either $ (1) $ or $ (2) $ with $ 1 $ or $ N_A-1 $ copies depending on if the $ \hat{\sigma}_{ba} $ operator is about the same atom as the other joint cumulant in the two-body moment variables. In deriving Eqs.~\eqref{eq:dsigmaba_QND} and~\eqref{eq:dsigmabadc_QND}, we have used the Gaussian state assumption to write three-body moments with one- and two-body moments by vanishing the corresponding third-order cumulants of operators--that is, $ \expect{\Delta A\Delta B\Delta C }=\expect{(\hat{A}-\expect{\hat{A}} )(\hat{B}-\expect{\hat{B}})(\hat{C}-\expect{\hat{C}}) } =0$ or $ \expect{\hat{A}\hat{B}\hat{C}}=\expect{\hat{A}\hat{B}}\expect{\hat{C}}+ \expect{\hat{A}\hat{C} }\expect{\hat{B}}+\expect{\hat{B}\hat{C} }\expect{\hat{A}}-2\expect{\hat{A}}\expect{\hat{B}}\expect{\hat{C}} $. With all these tricks applied, Eqs.~\eqref{eq:dsigmaba_QND} and~\eqref{eq:dsigmabadc_QND} yield
...
\end{align}
\end{subequations}
With the initial values and equations of motion given above, the stochastic time evolution of the one- and two-body moments can then be solved by evolving the equations from the initial value to arbitrary time $ t $. The collective spin squeezing dynamics can be obtained using the relations derived in Appendix~\ref{Appendix::collectivespinoperators} given the one-body and two-body moments calculated in this approach.

Our spin dynamics simulation results have been verified in the far-detuning regime against Eq.~\eqref{eq:op_fardetuning} and against the approximate equations of motions of one- and two-moments defined directly by the collective operators $ \hat{N}_A $ and $ \hat{F}_i $ ($ i=x,y,z $) following the derivation process introduced in Refs.~\cite{Qi2016} and~\cite{Norris2014}.
